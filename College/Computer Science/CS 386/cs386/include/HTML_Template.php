...
class HTML_Template
{
     ...
     */
    function HTML_Template($root = ".", $unknowns = "remove", $fallback="")
    {
        $this->setRoot($root);
		...
		{
			if(substr($key, 0, 5) == 'SITE_' || substr($key, 0, 5) == 'LINK_')
			{
	...
	function setBlocks($parent, $prefix = '', $suffix = '')
	{
        if (!$this->_loadFile($parent)) {
            $this->halt("setBlock: unable to load $parent.");
            return false;
		...
		foreach($m[1] as $index => $handle)
		{
			$this->setVar($prefix . $handle . $suffix, $m[2][$index]);
		}
	}
    ...
    {
        PEAR::raiseError(sprintf("<b>SITE_TEMPLATE Error:</b> %s<br>\n", $msg));
    }
}
?>
