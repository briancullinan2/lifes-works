...
/*void main () {
  ...
  P1OUT = 1;
  P1DIR = 1; // BIT1 - 00000001
  P1REN = 2; // BIT2 - 00000010
  for (;;) {
    if(P1IN & 2) { // button is off - active low
      P1OUT &= ~BIT1;
    } else { // button on
      P1OUT |= BIT1;
    }
...
// enable only what you have when you have to
/* enable interrupts
{
init
}
...
Active Mode
I = 300mA (3.3V)
Vec = 1.8 V (min for device)
I = 200mA

LPM0 - Disables CPU + MCLK
      not SMCLK, ACLK
I = 85mA
LPM3 - Only ACLK is active
I = 1uA

LPM4 - Everything off, external signal wake
I = 0.1uA

__low_power_mode_odd_on_exit();
